...
class Meal:
    ...
    def __init__(self, ingredients: list[str], name: str, description: str):
        """
        Initializes a new meal
        :param ingredients: The ingredients in this meal
        :param name: The name
        :param description: A descsription of this meal
        :raises TypeError: If ingredients is not a list of strings, name is not a string, or description is not a string
        :raises ValueError: If name is an empty string
        """
        if not isinstance(ingredients, list):
            raise TypeError("ingredients must be a list")
        if len([ingredient for ingredient in ingredients if not isinstance(ingredient, str)]) > 0:
            raise TypeError("All indexes of ingredients must be a string")
